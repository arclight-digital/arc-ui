...
export function generateUtilitiesCSS() {
  ...
  groups.push(
    section('Alignment', [
      ...['start', 'center', 'end', 'stretch', 'baseline'].map((v) =>
        rule(`items-${v}`, `align-items: ${v === 'start' || v === 'end' ? `flex-${v}` : v};`),
      ),
      ...
      ...['start', 'center', 'end', 'stretch', 'auto'].map((v) =>
        rule(`self-${v}`, `align-self: ${v === 'start' || v === 'end' ? `flex-${v}` : v};`),
      ),
    ]),
  );
  ...
  groups.push(
    section(
      'Font role',
      FONT_ROLES.map((role) => rule(`font-${role}`, `font-family: var(--font-${role});`)),
    ),
  );
