 ...
 *
 * One test is marked BUG: arc-option documents a `disabled` prop that this
 * control never reads. See test-findings.md.
 */
...
describe('arc-segmented-control disabled', () => {
  ...
  // BUG: arc-option declares `@prop {boolean} disabled` — "When true, dims this
  // option and prevents it from being selected" (shared/option.js:12). This
  // control never reads it: _readOptions (segmented-control.js:126) filters on
  // tagName only, _select has no per-option guard, _handleKeydown does not skip
  // disabled options, and render applies ?disabled from the *group* only. A
  // disabled option is fully selectable by click and by arrow key.
  it('BUG: a disabled arc-option is still selectable', async () => {
    const el = await control('', `
      <arc-option value="day">Day</arc-option>
    ...
    expect(el.value, 'documented behaviour is that this cannot be selected').to.equal('week');
    expect(seen).to.deep.equal([['change', 'week']]);

    // And the arrow keys walk onto it rather than skipping past.
    el.value = 'day';
    await settle(el);
    keyOn(buttons(el)[0], 'ArrowRight');
    await settle(el);
