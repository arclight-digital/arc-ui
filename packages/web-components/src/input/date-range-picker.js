...
import { DeclaredPropsMixin, flag, oneOf, list, int } from '../shared/props.js';
...
export class ArcDateRangePicker extends DeclaredPropsMixin(FormControlMixin(LitElement)) {
  ...
  static styles = [
    ...
    css`
      ...
      input:hover:not(:focus) { border-color: var(--border-bright); box-shadow: var(--shadow-inset), var(--interactive-hover); }
        ...
        >
          ${days.map((d) => {
            ...
            const suffix = isStart
              ...
              : isEnd
                ? ', range end'
                : !committed && (isLo || isHi) && range.lo !== range.hi
                  ? ', range end'
                  : '';
  ...
  render() {
    ...
    const allDays = panels
      .flatMap((p) => this._buildPanelDays(p.year, p.month))
      .filter((d) => !d.empty);
