...
import type { ArcThemeToggle } from '@arclux/arc-ui/theme-toggle';
...
export class ThemeToggle {
  ...
  @Output() themeChange = new EventEmitter<'dark' | 'light' | 'auto'>();

  _onArcChange(event: CustomEvent) {
    const detail = event.detail as Record<string, unknown> | null;
    if (!detail) return;
    if ('value' in detail) {
      this.themeChange.emit(detail.value as 'dark' | 'light' | 'auto');
    }
  }
}
