...
<script lang="ts">
  ...
  let { required, readonly, size, value = $bindable(0), min, max, step, name = '', disabled = false, label = '', detents, format, ...rest }: Props = $props();
   ...
   */
  export function element(): ArcKnob | undefined {
    return __el as ArcKnob | undefined;
  }
...
<arc-knob {required} {readonly} {size} {value} {min} {max} {step} {name} {disabled} {label} {detents} {format} bind:this={__el} {...rest}
