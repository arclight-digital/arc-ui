...
<script lang="ts">
  ...
  let { required, readonly, name = '', label = '', placeholder = '', value = $bindable(''), disabled = false, error = '', size, autocomplete = 'current-password', showStrength, ...rest }: Props = $props();
   ...
   */
  export function element(): ArcPasswordInput | undefined {
    return __el as ArcPasswordInput | undefined;
  }
...
<arc-password-input {required} {readonly} {name} {label} {placeholder} {value} {disabled} {error} {size} {autocomplete} bind:this={__el} {...rest}
