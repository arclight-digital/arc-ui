...
<script lang="ts">
  ...
  let { required, readonly, size, value = $bindable(''), name = '', min = '', max = '', placeholder = 'Select date', disabled = false, label = '', open = false, locale = '', firstDayOfWeek = 0, ...rest }: Props = $props();
   ...
   */
  export function element(): ArcDatePicker | undefined {
    return __el as ArcDatePicker | undefined;
  }
...
<arc-date-picker {required} {readonly} {size} {value} {name} {min} {max} {placeholder} {disabled} {label} {open} {locale} bind:this={__el} {...rest}
