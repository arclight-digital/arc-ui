...
<script lang="ts">
  ...
  let { required, readonly, size, locale = '', firstDayOfWeek = 0, start = $bindable(''), end = $bindable(''), name = '', min = '', max = '', months, presets, placeholder = 'Select date range', disabled = false, label = '', open = false, ...rest }: Props = $props();
   ...
   */
  export function element(): ArcDateRangePicker | undefined {
    return __el as ArcDateRangePicker | undefined;
  }
...
<arc-date-range-picker {required} {readonly} {size} {locale} {start} {end} {name} {min} {max} {months} {presets} {placeholder} {disabled} {label} {open} bind:this={__el} {...rest}
