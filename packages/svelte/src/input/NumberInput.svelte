...
<script lang="ts">
  ...
  let { required, readonly, size, value = $bindable(0), min, max, step = 1, label = '', name = '', disabled = false, ...rest }: Props = $props();
   ...
   */
  export function element(): ArcNumberInput | undefined {
    return __el as ArcNumberInput | undefined;
  }
...
<arc-number-input {required} {readonly} {size} {value} {min} {max} {step} {label} {name} {disabled} bind:this={__el} {...rest}
