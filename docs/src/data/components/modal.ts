...
@Component({
  ...
  template: \`
    <arc-button variant="secondary" (click)="open = true">Edit Profile</arc-button>
    <arc-modal [open]="open" heading="Edit Profile" size="md" (arcClose)="open = false">
      <div style="display:flex; flex-direction:column; gap:12px;">
        <label>Display Name<arc-input value="Ada Lovelace" /></label>
        <label>Bio<arc-input value="Analytical engine enthusiast" /></label>
      </div>
      <div slot="footer">
