...
export const guidedTour: ComponentDef = {
  ...
  overview: `GuidedTour orchestrates a multi-step onboarding experience by composing spotlight overlays with popover-styled tooltip panels at each step. It manages the full state machine — advancing, going back, skipping, and completing — so you only need to declare the steps and let the component handle transitions, positioning, and overlay management.
...
The component fires events at each transition — \`arc-change\` when the step changes, \`arc-complete\` when the user finishes the tour, and \`arc-close\` when they skip or close early. These events let you track onboarding completion, persist progress to a backend, or conditionally skip steps based on user state.`,
  ...
  guidelines: {
    ...
    dont: [
      'Do not create tours with more than 7 steps — break them into multiple focused tours',
      'Do not use guided-tour for error messages or alerts — it is for onboarding and discovery',
      'Do not target elements inside scrollable containers without ensuring they are visible',
      'Do not trigger the tour automatically on every page load — use a first-visit or feature-flag check',
      'Do not rely on the tour as the only form of documentation — it supplements, not replaces, help content',
    ],
  },
