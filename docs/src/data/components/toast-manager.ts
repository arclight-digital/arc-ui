...
Because a toaster is usually a singleton placed at the application root, the manager also listens for a document-level \`arc-toast\` CustomEvent. Any component, anywhere in the tree, can fire a toast without holding a ref: \`document.dispatchEvent(new CustomEvent('arc-toast', { detail: { message: 'Saved.', variant: 'success' } }))\`. The event detail is the same options object accepted by \`show()\`. The inner \`<arc-toast>\` retains full ownership of rendering and accessibility (\`role="status"\`, \`aria-live="polite"\`), so the manager itself renders nothing of its own (\`display: contents\`) and forwards its \`position\` and \`duration\` props straight through.`,
  ...
  guidelines: {
    ...
    dont: [
      'Do not nest an <arc-toast-manager> inside another, or mix it with a bare <arc-toast> on the same page — two toasters will overlap',
      'Do not set queue-limit very high to "never lose" notifications; stale toasts shown minutes later confuse more than they inform',
      'Do not disable dedupe and then fire per-item toasts for batch operations — summarize into one message instead',
      'Do not use the queue as a persistent notification inbox; use a NotificationPanel for history',
      'Do not rely on arc-queue-change for business logic — it describes UI state, not delivery guarantees',
    ],
  },
...
<template>
  ...
  <div style="display: flex; gap: 8px; flex-wrap: wrap;">
    ...
    },
    {
      ...
      lang: 'html',
      code: `<script>
  import { Button, ToastManager } from '@arclux/arc-ui-svelte';

  let manager;

  const save = () => manager?.show({ message: 'Changes saved.', variant: 'success' });
...
<ToastManager bind:this={manager} position="top-right" maxVisible={3} />
<div style="display: flex; gap: 8px; flex-wrap: wrap;">
  <Button variant="primary" on:click={save}>Show</Button>
  <Button variant="secondary" on:click={fireGlobal}>Fire via document event</Button>
  <Button variant="ghost" on:click={() => manager?.clear()}>Clear all</Button>
</div>`,
    },
    {
      label: 'Angular',
...
import { Button, ToastManager } from '@arclux/arc-ui-angular';

@Component({
  imports: [ToastManager, Button],
  template: \`
    <arc-toast-manager
      #toaster
      position="top-right"
      [maxVisible]="3"
      (arcQueueOverflow)="onOverflow($event)"
    ></arc-toast-manager>
    <div style="display: flex; gap: 8px; flex-wrap: wrap;">
      <arc-button variant="primary" (click)="save()">Show</arc-button>
      <arc-button variant="secondary" (click)="fireGlobal()">Fire via document event</arc-button>
      <arc-button variant="ghost" (click)="clearAll()">Clear all</arc-button>
    </div>
  \`,
})
export class ToastManagerDemoComponent {
  @ViewChild('toaster', { read: ElementRef }) toaster!: ElementRef;

  save() {
    this.toaster.nativeElement.show({ message: 'Changes saved.', variant: 'success' });
  }
  ...
  clearAll() {
    this.toaster.nativeElement.clear();
  }

  onOverflow(e: CustomEvent) {
    console.warn(e.detail.dropped, 'dropped');
  }
}`,
    },
  ],
